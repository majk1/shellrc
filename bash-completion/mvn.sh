...
_mvn()
{
    ...
    common_plugins="deploy|failsafe|install|site|surefire|checkstyle|javadoc|jxr|pmd|ant|antrun|archetype|assembly|dependency|enforcer|gpg|help|release|repository|source|eclipse|idea|jetty|cargo|jboss-as|jboss|wls|wildfly|tomcat|tomcat6|tomcat7|glassfish|embedded-glassfish|exec|versions|war|ear|ejb|android|scm|nexus|repository|sonar|license|hibernate3|liquibase|spring-boot"
    ...
    plugin_goals_cargo="cargo:start|cargo:run|cargo:stop|cargo:deploy|cargo:undeploy|cargo:redeploy|cargo:help"
    plugin_goals_jboss_as="jboss-as:deploy|jboss-as:redeploy|jboss-as:undeploy|jboss-as:run"
    plugin_goals_jboss="jboss:start|jboss:stop|jboss:deploy|jboss:undeploy|jboss:redeploy"
    plugin_goals_wls="wls:deploy|wls:redeploy|wls:undeploy"
    plugin_goals_wildfly="wildfly:deploy|wildfly:undeploy"
    plugin_goals_tomcat="tomcat:start|tomcat:stop|tomcat:deploy|tomcat:redeploy|tomcat:undeploy"
    plugin_goals_tomcat6="tomcat6:run|tomcat6:run-war|tomcat6:run-war-only|tomcat6:stop|tomcat6:deploy|tomcat6:redeploy|tomcat6:undeploy"
    plugin_goals_tomcat7="tomcat7:run|tomcat7:run-war|tomcat7:run-war-only|tomcat7:deploy"
    plugin_goals_glassfish="glassfish:run|glassfish:stop|glassfish:deploy|glassfish:redeploy|glassfish:undeploy"
    plugin_goals_embedded_glassfish="embedded-glassfish:run|embedded-glassfish:stop|embedded-glassfish:deploy|embedded-glassfish:redeploy|embedded-glassfish:undeploy"
    ...
    options="-Dmaven.test.failure.ignore=true|-Dmaven.test.skip=true|-DskipTests|-DskipITs|-Dmaven.surefire.debug|-DenableCiProfile|-Dpmd.skip=true|-Dcheckstyle.skip=true|-Dfile=|-DgroupID=|-DartifactId=|-Dversion=|-Dpackaging=|-Dsonar.exclusions=|-Dsonar.profile=|-DdryRun=true|-DskipDependencyVersionsCheck=true"
    ...
    local IFS=$'|\n'
			 ...
			 elif [[ ${cur} == -Dsonar.profile= ]] ; then
				 COMPREPLY=( $(compgen -d -S ' ' -- ${cur}) )

			 elif [[ ${cur} == -Dpackaging= ]] ; then
			 	 COMPREPLY=( $(compgen -S ' ' -W "jar|war" -- ${cur##*=}) )
